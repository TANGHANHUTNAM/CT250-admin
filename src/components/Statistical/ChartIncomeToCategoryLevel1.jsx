...
import { getTotalIncomesToCategoryLevel1 } from "../../services/statisticService";
...
const ChartInComeToSubcategory = () => {
  ...
  const fetchDataIncomeToSubcategory = async () => {
    try {
      const response = await getTotalIncomesToCategoryLevel1();
  ...
  const options = {
    ...
    plugins: {
      title: {
        display: true,
        text: "BIỂU ĐỒ THU NHẬP THEO DANH MỤC CẤP 1",
        font: {
